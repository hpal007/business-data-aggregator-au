...
def cc_business_info_extraction_dag():
    ...
    @task
    def process_files_with_duckdb(
        parquet_files: list[str], max_url_length: int = 80, check_existing: bool = True
    ):
        os.makedirs(DATA_DIR, exist_ok=True)

        # If check_existing flag is True, check for any existing files
        if check_existing:
            logger.info("checking for exising files..")
            existing_files = {}
            for relative_url in parquet_files[
                :30
            ]:  # Process only first 30 urls from 300 parquet_files  TODO update this for processing more parquet_files[31:60]
                ...
                if os.path.exists(output_path):
                    try:
                        ...
                        existing_files[relative_url] = row_count
                        con.close()
                    except Exception as e:
                        logger.error(
                            f"Error reading existing file {output_path}: {type(e).__name__}: {e}"
                        )
                        continue

            # If any files exist, return them and exit
            if existing_files:
                logger.info(
                    "Found existing files in DATA_DIR. Returning existing files."
                )
                logger.info(f"Total files found: {len(existing_files)}")
                logger.info(f"Total AU records: {sum(existing_files.values())}")
                return existing_files

        # Process files if check_existing is False or no existing files found
        con = duckdb.connect()
